...
int main(int argc, char** argv)
{
    ...
    SOCKET tSockFd;
    struct ip_mreq_source tMreq = { 0 };

    if (argc < 2)
    {
        printf("usage: ssmrecv \"souce ip\" ... source ip should > 1\n");
        return -1;
    }
    ...
    {
        tMreq.imr_sourceaddr.s_addr = inet_addr(argv[i]);
        nRet = setsockopt(tSockFd, IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP, (char*)&tMreq, sizeof(tMreq));
        if (-1 == nRet)
        {
            printf("setsockopt IP_ADD_SOURCE_MEMBERSHIP  error \n");
            return -1;
        }
    }
    ...
    {
        tMreq.imr_sourceaddr.s_addr = inet_addr(argv[i]);
        nRet = setsockopt(tSockFd, IPPROTO_IP, IP_DROP_SOURCE_MEMBERSHIP, (char*)&tMreq, sizeof(tMreq));
        if (-1 == nRet)
        {
            printf("setsockopt IP_DROP_SOURCE_MEMBERSHIP  error \n");
            return -1;
        }
    }
